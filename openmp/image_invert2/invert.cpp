    ...
    #pragma omp parallel shared(x) private(y,r,g,b,p)
    {
      #pragma omp for
        for (y=0; y < image->h; y++)
        {
          ...
          r = 255 - get_r(p);//(Uint32)(cos(p)*cos(y)*sin((p >> 16) & 0xff)*255.0f);
          g = 255 - get_g(p);//(Uint32)(cos(p)*cos(y)*sin((p >> 8) & 0xff)*255.0f);
          b = 255 - get_b(p);//(Uint32)(cos(p)*cos(y)*sin((p & 0xff))*255.0f);
